...
if not 'userInputs' in locals():
    ...
    userInputs['doBxOnly'] = True #bool(int(input('Do you want to analyze ONLY the behavioral data?: ')))
...
if userInputs['oneMouse']:
    ...
    lf.moveFiles(thisMouse,inputPath,fileType='.mp4')
    ...
    if not userInputs['doBxOnly']:
        ...
        print(currFolders)
        thisDay = currFolders[int(input('Which day from currFolders: '))-1] # THIS LINE SUBTRACTS 1 FROM THE RESPONSE TO CORRECT INPUT, ASSUMING THE USER IS INPUTTING THE MOUSE ID OF INTEREST AND NOT THE PYTHON INDEX FOR THAT MOUSE
    ...
    if not userInputs['pullMPC']:
        ...
        if userInputs['doBxOnly']:    
            ...
            for day in range(len(currFolders)):
                print(currFolders[day]); dataPath = inputPath + currFolders[day]
                # if the folder is empty, tag it and move on
    ...
    for inx in range(len(allDataSheet['Date'])):
        allDataSheet.at[inx,'Date'] = currFolders[inx]
...
#%% Part 2 - analyze data for each day    
    ...
    for day in range(len(allDataSheet)):
        ...
        tType[day] = allDataSheet.loc[day, 'trialType'] #1 go, 0 no, 2 tooEarly (tooEarly if trialOutcome=5)
        if not 2 in tType[day]:
            if isinstance(responseMatrix[day], np.ndarray):
                for trial in range(nTrials[day]):
                    if phaseIndex[day] == 1:
                        tType_calc[day] = np.append(tType_calc[day], 1)
                    elif tType[day][trial] == 1: # go
                        tType_calc[day] = np.append(tType_calc[day], 1)
                    elif tType[day][trial] == 0 and responseMatrix[day][trial] == 2 or responseMatrix[day][trial] == 4: # no-go 
                        tType_calc[day] = np.append(tType_calc[day], 0)
                    elif tType[day][trial] == 0 and responseMatrix[day][trial] == 5: # tooEarly
                        tType_calc[day] = np.append(tType_calc[day], 2)    
                    else:
                        tType_calc[day] = np.append(tType_calc[day], 3) # if not a go trial, no-go, or too early - effectively should never happen
            else:
                tType_calc[day] = tType[day]+1
        elif 2 in tType[day]:
...
######  time for some basic graphs (line for training trajectory)
    ...
    plt.plot(list(range(1, len(allDataSheet)+1)), rateAccuracy, color='purple', linewidth=10, markersize=15, marker='o')
    ...
    # Go trial accuracy
    plt.figure(figsize=(20, 12))
    plt.plot(list(range(1, len(allDataSheet)+1)), rateAccuracyGo, color='blue', linewidth=10, markersize=15, marker='o', label='go')
    plt.plot(list(range(1, len(allDataSheet)+1)), rateAccuracyNo, color='red', linewidth=10, markersize=15, marker='o', label='no-go')
    plt.axhline(0.65, color='black', linestyle='--', linewidth=2)
...
###### Now some scatter plots to align events to trial start
    ...
    rawBeamBreak_calc = [[] for _ in range(len(allDataSheet))]; tBeamBreak_calc = [[] for _ in range(len(allDataSheet))]; tBeamBreak_calcIndex = [[] for _ in range(len(allDataSheet))]; rawBeamBreak_calcOthers = [[] for _ in range(len(allDataSheet))]; tBeamBreak_calcOthers = [[] for _ in range(len(allDataSheet))];
    ...
    for day in range(len(allDataSheet)):
        if any(x<0 for x in tReward[day]):
            for trial in range(nTrials[day]):
                ...
                elif tReward[day][trial] < 0 and tType_calc[day][trial] == 0: #no reward during nogo trial
                ...
                elif tReward[day][trial] < 0 and tType_calc[day][trial] == 2: #too early                                             ^
    ...
    for day in range(len(allDataSheet)):
        if isinstance(tBeamBreak[day], np.ndarray):
            rawBeamBreak_calcOthers[day] = [[] for t in range(nTrials[day])]
            tBeamBreak_calcOthers[day] = [[] for t in range(nTrials[day])]
            for trial in range(nTrials[day]):
                afterStart = tBeamBreak[day]>rawStart[day][trial]
                if trial == len(rawStart[day])-1:
                    beforeEnd = (tBeamBreak[day]>rawStart[day][trial]) & (tBeamBreak[day]<rawEnd[day][trial])
                else:
                    beforeEnd = (tBeamBreak[day]>rawStart[day][trial]) & (tBeamBreak[day]<rawStart[day][trial+1])
                if not np.logical_and(afterStart, beforeEnd).any():
                    rawBeamBreak_calc[day] = np.append(rawBeamBreak_calc[day],-999) #no response
                    rawBeamBreak_calcOthers[day][trial] = -999 #tBeamBreak[day][np.logical_and(afterStart, beforeEnd)][1:]
                    tBeamBreak_calc[day] = np.append(tBeamBreak_calc[day],-999)
                    tBeamBreak_calcOthers[day][trial] = -999 #tBeamBreak_calc[day][np.logical_and(afterStart, beforeEnd)][1:]
                else:
                    rawBeamBreak_calc[day] = np.append(rawBeamBreak_calc[day], tBeamBreak[day][np.logical_and(afterStart, beforeEnd)][0])
                    rawBeamBreak_calcOthers[day][trial] = tBeamBreak[day][np.logical_and(afterStart, beforeEnd)][1:]
                    tBeamBreak_calc[day] = np.append(tBeamBreak_calc[day], rawBeamBreak_calc[day][trial]-rawStart[day][trial])           
                    for i in range(len(rawBeamBreak_calcOthers[day][trial])):
                        tBeamBreak_calcOthers[day][trial] = np.append(tBeamBreak_calcOthers[day][trial], rawBeamBreak_calcOthers[day][trial][i]-rawStart[day][trial])
...
###### Here I'm going to plot a PSTH of these response frequencies, so nTrials (not including tooEarly) bin into 500msec bits and averaged
    ...
    for day in range(len(allDataSheet)):
        ...
        for trial in range(nTrials[day]):
            if tBeamBreak_calc[day][trial] > 0:
                ...
                elif tBeamBreak_calc[day][trial] < 0.5:
                    psthArray_index[day] = np.append(psthArray_index[day], 2)
                else:
                    psthArray_index[day] = np.append(psthArray_index[day], tType_calc[day][trial])
    ...
    for day in range(len(allDataSheet)):    
        ...
        psthArray_respBinned2D_no[day] = np.sum(psthArray_resp2D[day][psthArray_index[day]==0], axis=0)
...
###### time to plot a line graph to show the change in response number across these sessions (similar to accuracy but different)
    ...
    if 2 in phaseIndex:
        nGo = np.round(np.linspace(phaseIndex.index(2),(len(phaseIndex) - 1 - phaseIndex[::-1].index(2)),((len(phaseIndex) - 1 - phaseIndex[::-1].index(2))-phaseIndex.index(2))+1)).astype(int)
        plt.figure(figsize=(7, 6))
        plt.xlabel('block (20 trials)')
        plt.ylabel('accuracy (%)')
    ...
    if 3 in phaseIndex:
        nNoGo = np.round(np.linspace(phaseIndex.index(3),(len(phaseIndex) - 1 - phaseIndex[::-1].index(3)),((len(phaseIndex) - 1 - phaseIndex[::-1].index(3))-phaseIndex.index(3)))).astype(int)
    ...
    if 4 in phaseIndex:
        nRev = np.round(np.linspace(phaseIndex.index(4),(len(phaseIndex) - 1 - phaseIndex[::-1].index(4)),((len(phaseIndex) - 1 - phaseIndex[::-1].index(4))-phaseIndex.index(4)))).astype(int)
        cMap=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']       
...
#%% Part 3 - Save out key data into a .py
    ...
    saveVar.update({name: value for name, value in globals().items() if isVar(name)})
    def isVar(name):
        return name.startswith('responseMatrix')
    saveVar.update({name: value for name, value in globals().items() if isVar(name)})
    def isVar(name):
        return name.startswith('phase')
    saveVar.update({name: value for name, value in globals().items() if isVar(name)})
    def isVar(name):
        return name.startswith('allDataSheet')
    saveVar.update({name: value for name, value in globals().items() if isVar(name)})
...
## going to plot the total number of days in each phase (this is going to be across mice)
    ...
    gOutputPath = 'S:\\Private\\Data\\Vignali cv105\\analysis\\MARS\\activeAnalysis\\'
...
## going to plot the accuracy from the first day of each phase across mice
    ...
    ## reaching
    gRateAccuracySpout=[[] for mice in range(len(allMice))]
    for thisMouse in range(len(allMice)):    
        nSpout = np.round(np.linspace(dataStruct[thisMouse]['phaseIndex'].index(1),(len(dataStruct[thisMouse]['phaseIndex']) - 1 - dataStruct[thisMouse]['phaseIndex'][::-1].index(1)),(1+(len(dataStruct[thisMouse]['phaseIndex']) - 1 - dataStruct[thisMouse]['phaseIndex'][::-1].index(1))-dataStruct[thisMouse]['phaseIndex'].index(1)))).astype(int)
        for i in range(len(nSpout)):
            gRateAccuracySpout[thisMouse].append(dataStruct[thisMouse]['nTrials'][nSpout[i]])
    
    cMap=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']       
        
    plt.figure(figsize=(20, 16))
    plt.title('Trials across spout sessions',fontsize=48); plt.xlabel('Session (#)',fontsize=36); plt.ylabel('Reward received (%)',fontsize=36)
    plt.tick_params(axis='both', labelsize=48); ax = plt.gca(); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False);
    ax.set_xticks((range(1, max([[(len(gRateAccuracySpout[m]))] for m in range(len(allMice))])[0]+1))); 
    ax.set_xlim(1, max([[(len(gRateAccuracySpout[m]))] for m in range(len(allMice))])[0]); ax.set_ylim(0, 180)
    
    for thisMouse in range(len(allMice)):    
        plt.plot(list(range(1, len(gRateAccuracySpout[thisMouse])+1)), gRateAccuracySpout[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', label=allMice[thisMouse])
        
    plt.legend(fontsize=24, title='mouse', loc='upper right', bbox_to_anchor=(1.15, 1));
    
    plt.savefig('{directory}fullSessionAccuracy_spoutPhase_allMice.pdf'.format(directory=gOutputPath), format='pdf', bbox_inches='tight', dpi=300)
    plt.show(); plt.close()
    ...
    for thisMouse in range(len(allMice)):    
        plt.plot(list(range(1, len(gRateAccuracyGo[thisMouse])+1)), gRateAccuracyGo[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', label=allMice[thisMouse])
    ...
    for thisMouse in range(len(allMice)):    
        plt.plot(list(range(1, len(gRateAccuracyNo_noGo[thisMouse])+1)), gRateAccuracyNo_noGo[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', linestyle='dashed')
        plt.plot(list(range(1, len(gRateAccuracyGo_noGo[thisMouse])+1)), gRateAccuracyGo_noGo[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', label=allMice[thisMouse])
    ...
    for thisMouse in range(len(allMice)):    
        plt.plot(list(range(1, len(gRateAccuracyNo_rev[thisMouse])+1)), gRateAccuracyNo_rev[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', linestyle='dashed')
        plt.plot(list(range(1, len(gRateAccuracyGo_rev[thisMouse])+1)), gRateAccuracyGo_rev[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', label=allMice[thisMouse])
    ...
    for thisMouse in range(len(allMice)):    
        plt.plot(list(range(1, len(gRateTooEarly_noGo[thisMouse])+1)), gRateTooEarly_noGo[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', label=allMice[thisMouse])
    ...
    for thisMouse in range(len(allMice)):    
        plt.plot(list(range(1, len(gRateTooEarly_rev[thisMouse])+1)), gRateTooEarly_rev[thisMouse], color=cMap[thisMouse], linewidth=10, markersize=15, marker='o', label=allMice[thisMouse])
...
#%% Part 5 - analysis post-unblinding (for groups/averages)       
    ...
    if txtFiles:
        ...
        plt.xlabel('phase',fontsize=48)
        plt.ylabel('accuracy (%)',fontsize=48)
        plt.title('Accuracy in first 20 trials',fontsize=36)   
        ax = plt.gca(); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False);
        ax.set_yticks(np.linspace(0, 1, 11));  
        plt.tick_params(axis='both', labelsize=48); ax.set_ylim(0, 0.7); 
        ...
        plt.figure(figsize=(20, 16))
        plt.axhline(0.65, color='black', linestyle='--', linewidth=2)
        plt.title('Accuracy across go sessions',fontsize=48); plt.xlabel('Session (#)',fontsize=48); plt.ylabel('Accuracy (%)',fontsize=36)
        plt.tick_params(axis='both', labelsize=48); ax = plt.gca(); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False);
        ax.set_xticks((range(1, max([[(len(gRateAccuracyGo[m]))] for m in range(len(allMice))])[0]+1))); ax.set_yticks(np.linspace(0,1,11)); 
        ax.set_xlim(1, max([[(len(gRateAccuracyGo[m]))] for m in range(len(allMice))])[0]); ax.set_ylim(0, 1)
        for thisGroup in range(len(groupedMice)):
            for theseMice in range(len(groupedMice[thisGroup][1:])):
                ...
                if theseMice==0:
                    plt.plot(list(range(1, len(gRateAccuracyGo[allMice.index(mouseID)])+1)), gRateAccuracyGo[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                else:
                    plt.plot(list(range(1, len(gRateAccuracyGo[allMice.index(mouseID)])+1)), gRateAccuracyGo[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o',)
        ...
        plt.figure(figsize=(20, 16))
        plt.axhline(0.65, color='black', linestyle='--', linewidth=2)
        plt.title('Accuracy across no-go sessions',fontsize=48); plt.xlabel('Session (#)',fontsize=48); plt.ylabel('Accuracy (%)',fontsize=36)
        plt.tick_params(axis='both', labelsize=48); ax = plt.gca(); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False);
        ax.set_xticks((range(1, max([[(len(gRateAccuracyNo_noGo[m]))] for m in range(len(allMice))])[0]+1))); ax.set_yticks(np.linspace(0,1,11)); 
        ax.set_xlim(1, max([[(len(gRateAccuracyNo_noGo[m]))] for m in range(len(allMice))])[0]); ax.set_ylim(0, 1)
        for thisGroup in range(len(groupedMice)):
            for theseMice in range(len(groupedMice[thisGroup][1:])):
                mouseID = groupedMice[thisGroup][1+theseMice]
                if theseMice==0:
                    plt.plot(list(range(1, len(gRateAccuracyGo_noGo[allMice.index(mouseID)])+1)), gRateAccuracyGo_noGo[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                    plt.plot(list(range(1, len(gRateAccuracyNo_noGo[allMice.index(mouseID)])+1)), gRateAccuracyNo_noGo[allMice.index(mouseID)], color=cMap[thisGroup], linestyle='dashed', linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                else:
                    plt.plot(list(range(1, len(gRateAccuracyGo_noGo[allMice.index(mouseID)])+1)), gRateAccuracyGo_noGo[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o',)
                    plt.plot(list(range(1, len(gRateAccuracyNo_noGo[allMice.index(mouseID)])+1)), gRateAccuracyNo_noGo[allMice.index(mouseID)], color=cMap[thisGroup], linestyle='dashed', linewidth=10, markersize=15, marker='o',)
        ...
        for thisGroup in range(len(groupedMice)):
            for theseMice in range(len(groupedMice[thisGroup][1:])):
                mouseID = groupedMice[thisGroup][1+theseMice]
                if theseMice==0:
                    plt.plot(list(range(1, len(gRateAccuracyGo_rev[allMice.index(mouseID)])+1)), gRateAccuracyGo_rev[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                    plt.plot(list(range(1, len(gRateAccuracyNo_rev[allMice.index(mouseID)])+1)), gRateAccuracyNo_rev[allMice.index(mouseID)], color=cMap[thisGroup], linestyle='dashed', linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                else:
                    plt.plot(list(range(1, len(gRateAccuracyGo_rev[allMice.index(mouseID)])+1)), gRateAccuracyGo_rev[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o',)
                    plt.plot(list(range(1, len(gRateAccuracyNo_rev[allMice.index(mouseID)])+1)), gRateAccuracyNo_rev[allMice.index(mouseID)], color=cMap[thisGroup], linestyle='dashed', linewidth=10, markersize=15, marker='o',)
        ...
        # separate go and no go here (no-go dashed)      
        plt.figure(figsize=(20, 16))
        plt.title('TooEarly across no-go sessions',fontsize=48); plt.xlabel('Session (#)',fontsize=48); plt.ylabel('TooEarly (%)',fontsize=36)
        plt.tick_params(axis='both', labelsize=48); ax = plt.gca(); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False);
        ax.set_xticks((range(1, max([[(len(gRateTooEarly_noGo[m]))] for m in range(len(allMice))])[0]+1))); ax.set_yticks(np.linspace(0,.6,7)); 
        ax.set_xlim(1, max([[(len(gRateTooEarly_noGo[m]))] for m in range(len(allMice))])[0]); ax.set_ylim(0, .6)
        ...
        for thisGroup in range(len(groupedMice)):
            for theseMice in range(len(groupedMice[thisGroup][1:])):
                mouseID = groupedMice[thisGroup][1+theseMice]
                if theseMice==0:
                    plt.plot(list(range(1, len(gRateTooEarly_noGo[allMice.index(mouseID)])+1)), gRateTooEarly_noGo[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                else:
                    plt.plot(list(range(1, len(gRateTooEarly_noGo[allMice.index(mouseID)])+1)), gRateTooEarly_noGo[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o',)
        ...
        for thisGroup in range(len(groupedMice)):
            for theseMice in range(len(groupedMice[thisGroup][1:])):
                mouseID = groupedMice[thisGroup][1+theseMice]
                if theseMice==0:
                    plt.plot(list(range(1, len(gRateTooEarly_rev[allMice.index(mouseID)])+1)), gRateTooEarly_rev[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o', label=groupedMice[thisGroup][0])
                else:
                    plt.plot(list(range(1, len(gRateTooEarly_rev[allMice.index(mouseID)])+1)), gRateTooEarly_rev[allMice.index(mouseID)], color=cMap[thisGroup], linewidth=10, markersize=15, marker='o',)
